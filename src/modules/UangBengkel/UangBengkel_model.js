...
module.exports = {
  getDataAll: (limit, offset, keywords, sort) => {
    return new Promise((resolve, reject) => {
      connection.query(
        `SELECT @no:=@no+1 AS nomor, s.nopolisi_stnk, u.user_name, u.user_email, u.user_nomor,b.id_bengkel , b.sisa_anggaran_bengkel FROM bengkel b JOIN user u ON b.id_user = u.id_user JOIN stnk s ON b.id_stnk = s.id_stnk JOIN (SELECT @no:=0) r WHERE s.nopolisi_stnk LIKE ? ORDER BY ${sort} LIMIT ? OFFSET ?`,
  ...
  getSumBengkel: () => {
    return new Promise((resolve, reject) => {
      connection.query(
        'SELECT SUM(sisa_anggaran_bengkel) as totalsemuabengkel FROM bengkel',
        (error, result) => {
          !error ? resolve(result) : reject(new Error(error))
        }
      )
  ...
  getDataByIdSTNK: (id) => {
    return new Promise((resolve, reject) => {
      connection.query(
        'SELECT * FROM bengkel WHERE id_bengkel = ?',
        id,
        (error, result) => {
          !error ? resolve(result) : reject(new Error(error))
          // console.log(result)
  ...
  updateData: (setData, id) => {
    ...
    return new Promise((resolve, reject) => {
      connection.query(
        'UPDATE bengkel SET ? WHERE id_bengkel = ?',
        [setData, id],
        (error, result) => {
          if (!error) {
            const newResult = {
  ...
  getDataCount: (keywords) => {
    return new Promise((resolve, reject) => {
      connection.query(
        'SELECT COUNT(*) AS total FROM bengkel WHERE id_bengkel LIKE ?',
        keywords,
        (error, result) => {
  ...
  deleteData: (id) => {
    return new Promise((resolve, reject) => {
      connection.query(
        'DELETE FROM bengkel WHERE id_bengkel = ?',
        id,
        (error, result) => {
          !error ? resolve(result) : reject(new Error(error))
        }
